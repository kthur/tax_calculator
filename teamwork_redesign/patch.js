...
const targetPath = path.join(__dirname, 'tax-calculator.js');
let content = fs.readFileSync(targetPath, 'utf8');

// 1. Define new helper and calculateComprehensiveIncome
const newComprehensiveIncome = `  // 1. Helper: Public Pension Deduction
  ...
  // 종합소득세 전체 계산 (6개 소득 종합과세 및 공제 한도 정밀 반영)
  calculateComprehensiveIncome(profile = {}) {
    ...
    const denominator = totalIncome + isaIncome + bondSeparated;
...
const newWrappers = `  // 3. Compatibility Wrappers
  ...
  calculateYearEndTax(opts) {
    const profile = Object.assign({}, opts, {
      wage: opts.totalSalary || 0,
      dependentsCount: opts.dependents || 0
    });
    return this.calculateComprehensiveIncome(profile);
  }`;

// Find calculateComprehensiveIncome and replace it
const startToken = '  // 종합소득세 전체 계산 (금융소득 세부 유형 및 벤처투자 소득공제 연동)';
const endToken = '  // 2. 양도소득세 계산 (장기보유특별공제 및 이월과세)';

const startIndex = content.indexOf(startToken);
const endIndex = content.indexOf(endToken);

if (startIndex === -1 || endIndex === -1) {
  console.error("Could not find start or end token for calculateComprehensiveIncome!");
  process.exit(1);
}

const beforeComp = content.substring(0, startIndex);
const afterComp = content.substring(endIndex);

content = beforeComp + newComprehensiveIncome + '\n\n' + afterComp;

// Find calculateYearEndTax and replace it
const startYeToken = '  // 4. 연말정산 정밀 계산';
const endYeToken = '// ----- new feature functions added by TAX NAVI expansion -----';

const startYeIndex = content.indexOf(startYeToken);
const endYeIndex = content.indexOf(endYeToken);

if (startYeIndex === -1 || endYeIndex === -1) {
  console.error("Could not find start or end token for calculateYearEndTax!");
  process.exit(1);
}

// Extract content up to start of calculateYearEndTax
const beforeYe = content.substring(0, startYeIndex);
// The end of calculateYearEndTax is followed by }; and newline and then the endYeToken.
// Let's find }; before the endYeToken.
const closingBraceIndex = content.lastIndexOf('};', endYeIndex);
if (closingBraceIndex === -1 || closingBraceIndex < startYeIndex) {
  console.error("Could not find closing }; for TaxCalculator!");
  process.exit(1);
}

const afterYe = content.substring(closingBraceIndex);

content = beforeYe + newWrappers + '\n' + afterYe;

fs.writeFileSync(targetPath, content, 'utf8');
console.log("File successfully refactored and written.");
