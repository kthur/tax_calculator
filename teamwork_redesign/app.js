...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateInputProgress() {
    const fields = [
      'inc-a-wage', 'inc-a-biz-gen-revenue', 'inc-a-biz-gen-expense',
      'inc-a-biz-rent-revenue', 'inc-a-biz-rent-expense',
      'inc-a-interest-dom', 'inc-a-interest-overseas',
      'inc-a-dividend-dom', 'inc-a-dividend-overseas',
      'inc-a-pension-pub', 'inc-a-pension-pri',
      'inc-a-other-revenue', 'inc-a-other-expense',
      'inc-b-wage', 'inc-b-biz-gen-revenue', 'inc-b-biz-gen-expense',
      'inc-b-biz-rent-revenue', 'inc-b-biz-rent-expense',
      'inc-b-interest-dom', 'inc-b-interest-overseas',
      'inc-b-dividend-dom', 'inc-b-dividend-overseas',
      'inc-b-pension-pub', 'inc-b-pension-pri',
      'inc-b-other-revenue', 'inc-b-other-expense',
      'inc-a-card', 'inc-b-card',
      'inc-a-pension', 'inc-b-pension',
  ...
  function parseTaxData(text) {
    ...
    const patterns = [
      ...
      { key: 'totalSalary',   regex: /총급여(?:액)?\s*[:\s]*(?:￦|원)?\s*\[?\s*([\d,]+)\s*\]?/, id: 'inc-a-wage' },
  ...
  function parseIncomeInputs() {
    const d = {
      // Spouse A
      aWage: parseVal("inc-a-wage"),
      aBizGenRevenue: parseVal("inc-a-biz-gen-revenue"),
      aBizGenExpense: parseVal("inc-a-biz-gen-expense"),
      aBizRentRevenue: parseVal("inc-a-biz-rent-revenue"),
      aBizRentExpense: parseVal("inc-a-biz-rent-expense"),
      aInterestDom: parseVal("inc-a-interest-dom"),
      aDividendDom: parseVal("inc-a-dividend-dom"),
      aInterestOverseas: parseVal("inc-a-interest-overseas"),
      aDividendOverseas: parseVal("inc-a-dividend-overseas"),
      aPensionPub: parseVal("inc-a-pension-pub"),
      aPensionPri: parseVal("inc-a-pension-pri"),
      aOtherRevenue: parseVal("inc-a-other-revenue"),
      aOtherExpense: parseVal("inc-a-other-expense"),
      // Spouse B
      bWage: parseVal("inc-b-wage"),
      bBizGenRevenue: parseVal("inc-b-biz-gen-revenue"),
      bBizGenExpense: parseVal("inc-b-biz-gen-expense"),
      bBizRentRevenue: parseVal("inc-b-biz-rent-revenue"),
      bBizRentExpense: parseVal("inc-b-biz-rent-expense"),
      bInterestDom: parseVal("inc-b-interest-dom"),
      bDividendDom: parseVal("inc-b-dividend-dom"),
      bInterestOverseas: parseVal("inc-b-interest-overseas"),
      bDividendOverseas: parseVal("inc-b-dividend-overseas"),
      bPensionPub: parseVal("inc-b-pension-pub"),
      bPensionPri: parseVal("inc-b-pension-pri"),
    ...
    Object.defineProperties(d, {
      aType: {
        get() { return (this.aWage >= this.aBizGenRevenue) ? 'wage' : 'business'; },
        configurable: true, enumerable: true
      },
      aSalary: {
        get() { return this.aType === 'wage' ? this.aWage : this.aBizGenRevenue; },
        configurable: true, enumerable: true
      },
      aFinancialGen: {
        get() { return this.aInterestDom + this.aDividendDom; },
        configurable: true, enumerable: true
      },
      aFinancialOverseas: {
        get() { return this.aInterestOverseas + this.aDividendOverseas; },
        configurable: true, enumerable: true
      },
      bType: {
        get() { return (this.bWage >= this.bBizGenRevenue) ? 'wage' : 'business'; },
        configurable: true, enumerable: true
      },
      bSalary: {
        get() { return this.bType === 'wage' ? this.bWage : this.bBizGenRevenue; },
        configurable: true, enumerable: true
      },
      bFinancialGen: {
        get() { return this.bInterestDom + this.bDividendDom; },
        configurable: true, enumerable: true
      },
      bFinancialOverseas: {
        get() { return this.bInterestOverseas + this.bDividendOverseas; },
        configurable: true, enumerable: true
      }
    });
  ...
  function validateIncomeInputs(d) {
    clearInlineErrors();
    const numericFields = [
      'aWage', 'aBizGenRevenue', 'aBizGenExpense', 'aBizRentRevenue', 'aBizRentExpense',
      'aInterestDom', 'aDividendDom', 'aInterestOverseas', 'aDividendOverseas',
      'aPensionPub', 'aPensionPri', 'aOtherRevenue', 'aOtherExpense',
      'bWage', 'bBizGenRevenue', 'bBizGenExpense', 'bBizRentRevenue', 'bBizRentExpense',
      'bInterestDom', 'bDividendDom', 'bInterestOverseas', 'bDividendOverseas',
      'bPensionPub', 'bPensionPri', 'bOtherRevenue', 'bOtherExpense',
      'aCard', 'bCard', 'aYellow', 'bYellow', 'aPension', 'bPension', 'aIrp', 'bIrp',
      'aIsaIncome', 'bIsaIncome', 'aBondSeparated', 'bBondSeparated',
      'aVentureInvestment', 'bVentureInvestment', 'aHousingSubscription', 'bHousingSubscription',
      'aHousingLoanRepay', 'bHousingLoanRepay'
    ];
    for (const field of numericFields) {
      if (d[field] < 0) {
        showInlineError("income-form-error", "모든 입력금액은 0원 이상이어야 합니다.");
        return false;
      }
    }

    // Check Spouse A ISA Sub Eligibility
    if (d.aIsaType === "sub") {
    ...
    // Check Spouse B ISA Sub Eligibility
    if (d.bIsaType === "sub") {
      const bOpts = buildSpouseCalcOpts(d, "b");
      const bRes = TaxCalculator.calculateComprehensiveIncome(bOpts);
      const totalIncomeVal = bRes.TotalComprehensiveIncome !== undefined ? bRes.TotalComprehensiveIncome : (bRes.totalIncome || 0);
      if (d.bWage > 50000000 && totalIncomeVal > 38000000) {
        showInlineError("income-form-error", "배우자 B ISA 서민형 자격 없음 (근로소득 5,000만 초과 및 종합소득 3,800만 초과)");
        return false;
      }
    }
  ...
  function buildSpouseCalcOpts(d, prefix) {
    ...
    return {
      ...
      wage: isA ? d.aWage : d.bWage,
      bizGenRevenue: isA ? d.aBizGenRevenue : d.bBizGenRevenue,
      bizGenExpense: isA ? d.aBizGenExpense : d.bBizGenExpense,
      bizRentRevenue: isA ? d.aBizRentRevenue : d.bBizRentRevenue,
      bizRentExpense: isA ? d.aBizRentExpense : d.bBizRentExpense,
      interestDom: isA ? d.aInterestDom : d.bInterestDom,
      dividendDom: isA ? d.aDividendDom : d.bDividendDom,
      interestOverseas: isA ? d.aInterestOverseas : d.bInterestOverseas,
      dividendOverseas: isA ? d.aDividendOverseas : d.bDividendOverseas,
      pensionPub: isA ? d.aPensionPub : d.bPensionPub,
      pensionPri: isA ? d.aPensionPri : d.bPensionPri,
  ...
  function runOptimizerAndRender(d, dependents) {
    const personAOptData = {
      wage: d.aWage,
      bizGenRevenue: d.aBizGenRevenue,
      bizGenExpense: d.aBizGenExpense,
      bizRentRevenue: d.aBizRentRevenue,
      bizRentExpense: d.aBizRentExpense,
      interestDom: d.aInterestDom,
      dividendDom: d.aDividendDom,
      interestOverseas: d.aInterestOverseas,
      dividendOverseas: d.aDividendOverseas,
      pensionPub: d.aPensionPub,
      pensionPri: d.aPensionPri,
      otherRevenue: d.aOtherRevenue,
      otherExpense: d.aOtherExpense,
      card: d.aCard,
      pension: d.aPension,
      irp: d.aIrp,
      yellowUmbrella: d.aYellow,
      ventureInvestment: d.aVentureInvestment,
      housingSubscription: d.aHousingSubscription,
      housingLoanRepay: d.aHousingLoanRepay,
    ...
    const personBOptData = {
      wage: d.bWage,
      bizGenRevenue: d.bBizGenRevenue,
      bizGenExpense: d.bBizGenExpense,
      bizRentRevenue: d.bBizRentRevenue,
      bizRentExpense: d.bBizRentExpense,
      interestDom: d.bInterestDom,
      dividendDom: d.bDividendDom,
      interestOverseas: d.bInterestOverseas,
      dividendOverseas: d.bDividendOverseas,
      pensionPub: d.bPensionPub,
      pensionPri: d.bPensionPri,
      otherRevenue: d.bOtherRevenue,
      otherExpense: d.bOtherExpense,
      card: d.bCard,
      pension: d.bPension,
      irp: d.bIrp,
      yellowUmbrella: d.bYellow,
      ventureInvestment: d.bVentureInvestment,
      housingSubscription: d.bHousingSubscription,
      housingLoanRepay: d.bHousingLoanRepay,
  ...
  function renderAdviceSection(d, aResult) {
    ...
    renderAdvice("income-advice-list", [...incomeAdvice, ...yearEndAdvice], (id, val) => {
      if (id === "income_yellow_umbrella") { setAndFormatVal("inc-a-yellow", val); }
        ...
        let newInterestDom = d.aInterestDom;
        let newDividendDom = d.aDividendDom;
        if (newInterestDom >= remainingToReduce) {
          newInterestDom -= remainingToReduce;
          remainingToReduce = 0;
        } else {
          remainingToReduce -= newInterestDom;
          newInterestDom = 0;
        }
        if (remainingToReduce > 0) {
          newDividendDom = Math.max(0, newDividendDom - remainingToReduce);
        }
        setAndFormatVal("inc-a-interest-dom", newInterestDom);
        setAndFormatVal("inc-a-dividend-dom", newDividendDom);
      }
  ...
  function renderCardNavigation(d) {
    const aThreshold = Math.floor(d.aWage * 0.25);
    const bThreshold = Math.floor(d.bWage * 0.25);
    const aCardLimit = d.aWage > 120000000 ? 2000000 : d.aWage > 70000000 ? 2500000 : 3000000;
    const bCardLimit = d.bWage > 120000000 ? 2000000 : d.bWage > 70000000 ? 2500000 : 3000000;
  ...
  function renderMedicalComparison(d, dependents) {
    ...
    const aMed = Math.max(0, Math.floor((totalMedical - Math.floor(d.aWage * 0.03)) * 0.15));
    const bMed = Math.max(0, Math.floor((totalMedical - Math.floor(d.bWage * 0.03)) * 0.15));
  ...
  const btnCalcHiEl = document.getElementById('btn-calc-health-insurance');
  if (btnCalcHiEl) {
    btnCalcHiEl.addEventListener('click', () => {
  ...
  [
    ...
    'inc-b-pension-pub', 'inc-b-pension-pri',
    'inc-b-other-revenue', 'inc-b-other-expense',
    'inc-a-card', 'inc-b-card', 'inc-a-yellow', 'inc-b-yellow',
    'inc-a-pension', 'inc-b-pension', 'inc-a-irp', 'inc-b-irp',
    'inc-a-isa', 'inc-b-isa', 'inc-a-isa-type', 'inc-b-isa-type',
    'inc-a-bond', 'inc-b-bond',
    'inc-a-venture', 'inc-b-venture',
    'inc-a-housing-sub', 'inc-b-housing-sub',
    'inc-a-housing-loan', 'inc-b-housing-loan'
  ].forEach(id => {
    const el = document.getElementById(id);
